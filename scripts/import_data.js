...
// --- 配置 ---
// Assuming run from project root: node scripts/import_data.js
// JSON file expected in root
const JSON_FILE = 'data_export.json'; 
// DB is in root/data
const DB_PATH = path.join(__dirname, '../data', 'invest_track_v2.db');

// Check if file exists relative to cwd (root) or relative to script? 
// fs.readFileSync uses path relative to cwd if not absolute.
// Let's assume cwd is project root.
const jsonPathRaw = path.resolve(process.cwd(), JSON_FILE);

if (!fs.existsSync(jsonPathRaw)) {
...
const rawData = fs.readFileSync(jsonPathRaw, 'utf8');
