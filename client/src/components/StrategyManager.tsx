...
const StrategyManager: React.FC<StrategyManagerProps> = ({ strategies: versions, assets, onUpdate }) => {
  ...
  const submitLayerModal = (e: React.FormEvent) => {
      ...
      if (modalLayer.layerId) {
          // Edit
          newLayers = newLayers.map((l: StrategyLayer) => l.id === modalLayer.layerId ? { 
  ...
  const handleDeleteLayer = (layerId: string) => {
      ...
      if (!currentVersion) return;
      const newLayers = currentVersion.layers.filter((l: StrategyLayer) => l.id !== layerId);
      updateCurrentVersion(newLayers);
  };
  ...
  const submitAssetModal = (e: React.FormEvent) => {
      ...
      const assetObj = assets.find((a: Asset) => a.id === modalAsset.assetId);
      if (!assetObj) return;

      const newLayers = currentVersion.layers.map((l: StrategyLayer) => {
          if (l.id !== modalAsset.layerId) return l;
          
          let newItems = [...l.items];
          if (modalAsset.item) {
              // Edit
              newItems = newItems.map((i: StrategyTarget) => i.id === modalAsset.item!.id ? {
                  ...i,
              ...
              // Add
              if (newItems.find((i: StrategyTarget) => i.assetId === modalAsset.assetId)) { alert('该层级已包含此资产'); return l; }
              newItems.push({
                  id: generateId(),
                  assetId: modalAsset.assetId,
                  targetName: assetObj.name,
  ...
  const handleDeleteAsset = (layerId: string, itemId: string) => {
      ...
      const newLayers = currentVersion.layers.map((l: StrategyLayer) => {
          ...
          return { ...l, items: l.items.filter((i: StrategyTarget) => i.id !== itemId) };
      });
  ...
  const availableAssetsForModal = useMemo(() => {
    ...
    currentVersion.layers.forEach((layer: StrategyLayer) => {
        layer.items.forEach((item: StrategyTarget) => {
  ...
  const remainingWeightInModalLayer = useMemo(() => {
    ...
    const layer = currentVersion.layers.find((l: StrategyLayer) => l.id === modalAsset.layerId);
    if (!layer) return 0;
          ...
          <div className="flex flex-col h-full space-y-6 overflow-y-auto pr-2 pb-10 scrollbar-thin scrollbar-thumb-slate-200">
            ...
            <div>
                ...
                <div className="space-y-6">
                    {currentVersion.layers.map((layer: StrategyLayer) => {
                        // Calculate Distribution Visualization for this Layer
                        const fixedItems = layer.items.filter((i: StrategyTarget) => i.weight >= 0);
                        const autoItems = layer.items.filter((i: StrategyTarget) => i.weight === -1);
                        const usedWeight = fixedItems.reduce((sum: number, i: StrategyTarget) => sum + i.weight, 0);
                        ...
                        <div key={layer.id} className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                            ...
                            <div className="p-4">
                                <div className="space-y-2">
                                    {layer.items.map((item: StrategyTarget) => {
