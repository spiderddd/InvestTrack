...
该镜像采用多阶段构建，同时包含了前端界面和后端数据库服务。

### 1. 构建镜像

在项目根目录下（包含 Dockerfile 的目录）运行：
...
1.  **代码准备**: 将项目所有文件上传到 NAS 的某个文件夹。
2.  **构建**: 在 NAS 的 Docker/Container Manager 中，选择“构建镜像”或通过 SSH 执行上述 build 命令。
3.  **运行**: 创建容器时：
    *   **端口映射**: 容器端口 `3001` -> 本地端口 `3001` (或任意空闲端口)。
    *   **存储空间**: 将 NAS 上的文件夹挂载到容器的 `/app/data` 路径。
...
*   **前端 (Frontend)**: React 18, TypeScript, Vite, Tailwind CSS (via CDN), Recharts
*   **后端 (Backend)**: Node.js, Express, SQLite3

---

## 🚀 传统开发指南 (Non-Docker Development)
