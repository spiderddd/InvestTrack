...
│   └── index.js              # 后端入口文件
├── shared/                    # 前后端共享
│   └── types.ts              # TypeScript 类型定义 (API Contract)
├── scripts/                   # 数据维护脚本
│   ├── seed_data.js          # 测试数据生成
│   └── import_data.js        # 数据导入
...
- **明确分层**: `client/` 只放前端，`server/` 只放后端，边界清晰
- **共享类型**: `shared/types.ts` 作为前后端共同的 API 契约，避免"前端猜后端字段"
- **路径别名**: 使用 `@shared/*` 路径别名导入共享类型，简化导入路径
- **独立构建**: 前端通过 Vite 构建，输出到 `dist/` 目录由后端静态托管
...
npm run dev
```
*   Vite 开发服务器启动在 `http://localhost:5173`，自动代理 API 请求到后端。

### 3. 构建生产版本
```bash
npm run build
```
*   TypeScript 类型检查通过后，Vite 将构建前端到 `dist/` 目录
*   生产环境只需运行 `npm start` 即可同时托管前端静态文件和后端 API
